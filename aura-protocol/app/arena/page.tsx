...
export default function ArenaPage({ profile, initialBounties, squadFriends }: any) {
  ...
  async function handleIssueBounty(e: React.FormEvent) {
    ...
    if (profile.aura_points < wagerAmount) {
    ...
    try {
      ...
      const newAP = profile.aura_points - wagerAmount;
      await supabase.from("profiles").update({ aura_points: newAP }).eq("id", profile.id);
    ...
    <div className="max-w-md mx-auto p-4 space-y-6 pb-28">
      <div className="flex items-center justify-between">
        ...
        <div className="liquid-glass px-3 py-1.5 rounded-2xl flex items-center gap-1.5 text-xs font-black text-emerald-600 bg-white/70">
          ...
          <span>{profile.aura_points} AP</span>
        </div>
      </div>
