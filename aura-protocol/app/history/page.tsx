...
export default async function HistoryPage({
  ...
  const currentTab = resolvedParams?.tab === "work" ? "work" : "gym";
  const activityKey = currentTab === "work" ? "editing_deep_work" : "gym_workout";
  const proofActivityKey = currentTab === "work" ? "deep_work" : "gym";
  const isWork = currentTab === "work";

  // 1. Fetch time logs for this activity
  const { data: rawLogs } = await supabase
    .from("time_logs")
    .select("id, duration_seconds, log_date, notes, created_at")
    .eq("user_id", user.id)
  ...
  // 2. Fetch verified camera proofs logged for this activity
  const { data: rawProofs } = await supabase
    .from("posts")
    .select("id, image_url, caption, created_at")
    .eq("user_id", user.id)
    .eq("activity", proofActivityKey)
  ...
  // 4. Build Last 7 Days Visual Bar Chart Data
  ...
  // 5. Circular Progress / Goal Ring (10 hrs work = 600m, 5 hrs gym = 300m)
  const weeklyTargetMins = isWork ? 600 : 300;
  ...
  return (
    <div className="pt-10 px-4 pb-32 min-h-screen space-y-6 max-w-md mx-auto">
      ...
      {/* VISUAL ARCHIVE: Verified Camera Proofs */}
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1.5">
            <Camera className="w-3.5 h-3.5" />
            <span>Verified Proofs ({proofs.length})</span>
          </h3>
          <span className="text-[10px] text-zinc-400 font-medium">Last 6 Submissions</span>
        </div>
        ...
        {proofs.length > 0 ? (
          <div className="grid grid-cols-3 gap-2">
            {proofs.map((proof) => (
              <div
                key={proof.id}
                className="relative aspect-square rounded-2xl overflow-hidden border border-zinc-200/80 shadow-sm group bg-zinc-100"
              >
                <Image
                  src={proof.image_url}
                  alt={proof.caption || "Proof record"}
                  fill
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-end p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="text-[9px] text-white font-bold truncate">
                    {new Date(proof.created_at).toLocaleDateString([], { month: "short", day: "numeric" })}
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="liquid-glass p-6 rounded-2xl text-center text-zinc-400 text-xs font-medium border border-white/80 bg-white/60">
            No verified camera proofs logged for {isWork ? "deep work" : "workouts"} yet.
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-400 px-1 flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5" />
          <span>Recent {isWork ? "Deep Work" : "Workout"} Entries</span>
        </h3>
        ...
        {logs.length > 0 ? (
          logs.slice(-5).reverse().map((log) => {
            ...
            return (
              <div
                key={log.id}
                className="liquid-glass p-4 rounded-2xl flex items-center justify-between border border-white/80 shadow-sm backdrop-blur-xl bg-white/70"
              >
                <div>
                  <p className="text-sm font-bold text-zinc-800">
                    {log.notes || (isWork ? "Focus Session" : "Training Block")}
                  </p>
