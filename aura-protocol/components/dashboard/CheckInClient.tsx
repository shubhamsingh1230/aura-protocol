...
export default function CheckInClient({ profile, initialLog, gesture, analytics }: any) {
  ...
  async function awardProfilePoints(apDelta: number, xpDelta: number) {
    const updatedAP = Math.max(0, (userProfile?.aura_points || auraPoints || 0) + apDelta);
  ...
  async function handleGymVerification() {
    ...
    if (!gymFile) {
      alert("Photo proof is required for gym verification!");
      return;
    }
    setLoading(true);
  ...
  const mealsLoggedCount = log?.meals_logged || 0;
  ...
  return (
    <div className="pt-10 px-4 pb-32 min-h-screen space-y-4 max-w-md mx-auto">
      ...
      <div className="flex items-center justify-between px-2 mb-2">
        <div>
          ...
          <p className="text-zinc-500 text-xs font-semibold">
            @{userProfile?.handle || profile?.handle || "operator"} • {identityRank}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="liquid-glass px-3 py-1.5 rounded-2xl flex items-center gap-1.5 border border-white/80 shadow-sm text-xs font-bold text-orange-600">
            <Flame className="w-4 h-4 fill-orange-500" />
            <span>{currentStreak}d</span>
          </div>
          <div className="liquid-glass px-3 py-1.5 rounded-2xl flex items-center gap-1.5 border border-white/80 shadow-sm text-xs font-bold text-emerald-600">
            <Sparkles className="w-3.5 h-3.5" />
            <span>{userProfile?.aura_points ?? auraPoints} AP</span>
          </div>
        </div>
      </div>

      {/* TOP CARD: DAILY GAUNTLET */}
      <div className="liquid-glass rounded-3xl p-5 flex items-center justify-between border border-white/80 shadow-sm backdrop-blur-xl bg-white/70">
        <div className="flex-1">
          ...
          <p className="text-[11px] text-zinc-400 font-bold uppercase tracking-wider">
            {pillarsCompleteCount} of 7 Pillars Verified
          </p>
        </div>
        
        <div className="relative w-28 h-28 flex items-center justify-center">
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
            <circle cx="18" cy="18" r={radius} fill="none" stroke="rgba(0,0,0,0.06)" strokeWidth="4" />
            <circle 
              cx="18" 
              cy="18" 
              r={radius} 
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            ...
            <span className="text-[10px] font-black text-zinc-800">{calculatedDailyScore}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link href="/history?tab=work" className="liquid-glass rounded-3xl p-4 flex flex-col justify-between aspect-square active:scale-95 transition-all border border-white/80 shadow-sm backdrop-blur-xl bg-white/70">
          <div>
            <div className="flex items-center justify-between mb-1">
              <p className="text-zinc-800 font-bold text-xs tracking-tight">Deep Work</p>
              <ChevronRight className="w-3.5 h-3.5 text-zinc-400" />
            </div>
            <p className="text-[10px] text-zinc-500 font-medium">Logged Today</p>
            <p className="text-xl font-bold tabular-nums text-blue-600 mt-1">{editingTime}</p>
          </div>
        ...
        <Link href="/history?tab=gym" className="liquid-glass rounded-3xl p-4 flex flex-col justify-between aspect-square active:scale-95 transition-all border border-white/80 shadow-sm backdrop-blur-xl bg-white/70">
          <div>
            <div className="flex items-center justify-between mb-1">
              <p className="text-zinc-800 font-bold text-xs tracking-tight">Training</p>
              <ChevronRight className="w-3.5 h-3.5 text-zinc-400" />
            </div>
            <p className="text-[10px] text-zinc-500 font-medium">Logged Today</p>
            <p className="text-xl font-bold tabular-nums text-emerald-600 mt-1">{gymTime}</p>
          </div>
          <div className="flex items-end justify-between h-10 gap-1 mt-2">
        ...
        <div className="liquid-glass rounded-3xl p-4 aspect-square flex flex-col justify-center items-center text-center relative overflow-hidden border border-white/80 shadow-sm backdrop-blur-xl bg-white/70">
      ...
      <div className="mt-4">
        <h2 className="text-sm font-bold text-zinc-900 mb-2 px-2 uppercase tracking-wider">
        ...
        <div className="liquid-glass rounded-3xl p-2 flex flex-col gap-2 border border-white/80 shadow-sm backdrop-blur-xl bg-white/70">
          ...
          <div className={`rounded-2xl transition-all overflow-hidden border ${isGymDone ? "bg-emerald-500/5 border-emerald-500/20" : "bg-white/60 border-zinc-100"}`}>
            <button onClick={() => setActiveAction(activeAction === "gym" ? null : "gym")} className="flex items-center justify-between p-3.5 w-full text-left">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-emerald-500/10 text-emerald-600 rounded-xl"><Dumbbell className="w-4 h-4" /></div>
                <div>
                  <p className="text-xs font-bold text-zinc-800">Physical Training</p>
            ...
            {activeAction === "gym" && !isGymDone && (
              <div className="px-3 pb-3 pt-1 space-y-3">
                <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-200 border-dashed text-center flex flex-col items-center gap-1.5">
                  ...
                  <p className="text-xs text-zinc-600 font-medium">Show today&apos;s gesture: <strong className="text-zinc-900">{typeof gesture === "string" ? gesture : (gesture?.name || gesture?.gesture_name || "Peace Sign")}</strong></p>
                ...
                <button onClick={handleGymVerification} disabled={loading || !gymFile} className={`w-full py-2.5 text-white font-bold text-xs rounded-xl transition-all shadow-sm ${gymFile ? "bg-zinc-900 hover:bg-zinc-800" : "bg-zinc-300 cursor-not-allowed"}`}>
                  {loading ? "Compressing & Uploading..." : "Upload & Verify Workout"}
                </button>
              </div>
            )}
          </div>
          ...
          <div className={`rounded-2xl transition-all overflow-hidden border ${isWorkDone ? "bg-blue-500/5 border-blue-500/20" : "bg-white/60 border-zinc-100"}`}>
            <button onClick={() => setActiveAction(activeAction === "work" ? null : "work")} className="flex items-center justify-between p-3.5 w-full text-left">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-blue-500/10 text-blue-600 rounded-xl"><MonitorPlay className="w-4 h-4" /></div>
                <div>
                  <p className="text-xs font-bold text-zinc-800">Deep Work Block (4h+)</p>
            ...
            {activeAction === "work" && !isWorkDone && (
              <div className="px-3 pb-3 pt-1 space-y-3">
                ...
                <div className="flex gap-2">
                  <Link href="/time" className="flex-1 py-2.5 bg-blue-50 text-blue-600 border border-blue-200 text-center font-bold text-xs rounded-xl transition-all flex items-center justify-center">
                    Timer
                  </Link>
                  <button onClick={handleWorkVerification} disabled={loading || !workFile} className={`flex-1 py-2.5 text-white font-bold text-xs rounded-xl transition-all shadow-sm ${workFile ? "bg-zinc-900 hover:bg-zinc-800" : "bg-zinc-300 cursor-not-allowed"}`}>
          ...
          <div className={`rounded-2xl transition-all overflow-hidden border ${isNutritionDone ? "bg-orange-500/5 border-orange-500/20" : "bg-white/60 border-zinc-100"}`}>
            <button onClick={() => setActiveAction(activeAction === "meals" ? null : "meals")} className="flex items-center justify-between p-3.5 w-full text-left">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-orange-500/10 text-orange-600 rounded-xl"><Utensils className="w-4 h-4" /></div>
                <div>
            ...
            {activeAction === "meals" && mealsLoggedCount < 5 && (
              <div className="px-3 pb-3 pt-1 space-y-3">
                ...
                <button onClick={handleMealLogging} disabled={loading || !mealFile} className={`w-full py-2.5 text-white font-bold text-xs rounded-xl transition-all shadow-sm ${mealFile ? "bg-orange-500 hover:bg-orange-600" : "bg-zinc-300 cursor-not-allowed"}`}>
      ...
      <div className="mt-4">
        ...
        <div className="liquid-glass rounded-3xl p-2 flex flex-col gap-1.5 border border-white/80 shadow-sm backdrop-blur-xl bg-white/70">
          ...
          <div 
            ...
            className={`p-3.5 rounded-2xl cursor-pointer flex items-center justify-between transition-all border ${isMorningDone ? "bg-emerald-500/5 border-emerald-500/20" : "bg-white/60 border-zinc-100 hover:bg-white/80"}`}
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-amber-500/10 text-amber-600 rounded-xl"><Sun className="w-4 h-4" /></div>
              <div>
            ...
            {togglingKey === "morning_routine" ? <Loader2 className="w-4 h-4 animate-spin text-emerald-500" /> : isMorningDone ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <Circle className="w-5 h-5 text-zinc-300" />}
          </div>

          {/* Active Learning */}
          <div 
            onClick={() => handleTogglePillar("learning", 5, 15)}
            className={`p-3.5 rounded-2xl cursor-pointer flex items-center justify-between transition-all border ${isLearningDone ? "bg-emerald-500/5 border-emerald-500/20" : "bg-white/60 border-zinc-100 hover:bg-white/80"}`}
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-indigo-500/10 text-indigo-600 rounded-xl"><BookOpen className="w-4 h-4" /></div>
              <div>
                <p className="text-xs font-bold text-zinc-800">Active Learning Block</p>
                <p className="text-[10px] text-zinc-400 font-medium">+5 AP • +15 XP • 10% Weight</p>
              </div>
            </div>
            {togglingKey === "learning" ? <Loader2 className="w-4 h-4 animate-spin text-emerald-500" /> : isLearningDone ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <Circle className="w-5 h-5 text-zinc-300" />}
          </div>

          {/* Sleep Target */}
          <div 
            onClick={() => handleTogglePillar("sleep_target", 5, 15)}
            className={`p-3.5 rounded-2xl cursor-pointer flex items-center justify-between transition-all border ${isSleepDone ? "bg-emerald-500/5 border-emerald-500/20" : "bg-white/60 border-zinc-100 hover:bg-white/80"}`}
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-purple-500/10 text-purple-600 rounded-xl"><Moon className="w-4 h-4" /></div>
              <div>
                <p className="text-xs font-bold text-zinc-800">Sleep Target (7h+ Rest)</p>
                <p className="text-[10px] text-zinc-400 font-medium">+5 AP • +15 XP • 10% Weight</p>
              </div>
            </div>
            {togglingKey === "sleep_target" ? <Loader2 className="w-4 h-4 animate-spin text-emerald-500" /> : isSleepDone ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <Circle className="w-5 h-5 text-zinc-300" />}
          </div>

          {/* Evening Review */}
          <div 
            onClick={() => handleTogglePillar("daily_review", 5, 10)}
            className={`p-3.5 rounded-2xl cursor-pointer flex items-center justify-between transition-all border ${isReviewDone ? "bg-emerald-500/5 border-emerald-500/20" : "bg-white/60 border-zinc-100 hover:bg-white/80"}`}
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-teal-500/10 text-teal-600 rounded-xl"><ClipboardCheck className="w-4 h-4" /></div>
              <div>
            ...
            {togglingKey === "daily_review" ? <Loader2 className="w-4 h-4 animate-spin text-emerald-500" /> : isReviewDone ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <Circle className="w-5 h-5 text-zinc-300" />}
          </div>
